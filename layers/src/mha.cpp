...
namespace my_vllm 
{
MultiHeadAttention::MultiHeadAttention(DeviceType device_type, int32_t layer_index,
...
Status MultiHeadAttention::forward() 
{
    auto status = check();
    if (!status)
    {
    ...
    const Tensor& mha_out = this->get_output(0);
    const Tensor& query_tensor = this->get_input(0);
    const Tensor& score_tensor = this->get_input(1);
    const Tensor& key_cache_tensor = this->get_input(2);
    const Tensor& value_cache_tensor = this->get_input(3);

    if (device_type_ == DeviceType::kDeviceCUDA) 
    {
        CHECK(cuda_config_ != nullptr);
    }
    get_mha_kernel(device_type_)(pos_, head_num_, layer_index_, seq_len_, kv_dim_, kv_mul_,
...
Status MultiHeadAttention::check() const 
{
    Status status;
