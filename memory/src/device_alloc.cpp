                             ...
                             MemcpyKind memcpy_kind, void* stream, bool need_sync) const 
{
    ...
    else if (memcpy_kind == MemcpyKind::kMemcpyCPU2CUDA) 
    {
        if (!stream_) 
        {
            cudaMemcpy(dest_ptr, src_ptr, byte_size, cudaMemcpyHostToDevice); //cpu等待拷贝完成才往后走
        } 
        else
        {
            cudaMemcpyAsync(dest_ptr, src_ptr, byte_size, cudaMemcpyHostToDevice, stream_);
            //把任务放在stream流里面，gpu自己去完成，cpu不等待去干别的事情
        }
    } 
...
{
    CHECK(device_type_ != my_vllm::DeviceType::kDeviceUnknown);
    if (device_type_ == my_vllm::DeviceType::kDeviceCPU)
    {
