...
Status LLama2Model::forward(const Tensor& input, const Tensor& pos_tensor,
                                  int& next) const {
  if (input.is_empty()) {
...
void LLama2Model::create_param_layers() {
  ...
  llama_layers_->cls_layer_ =
      std::make_shared<MatmulLayer>(device_type_, config_->vocab_size_, dim);
  if (config_->is_shared_weight_) {
    // using token embedding weight
...
EmbeddingOutput LLama2Model::embedding(const std::vector<int>& tokens) const {
  ...
  auto input_token_num =
      Tensor(DataType::kDataTypeInt32, static_cast<int32_t>(tokens.size()));
